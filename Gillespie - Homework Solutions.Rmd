---
title: "INFO 550 Homework Solutions"
...
3.) Establishes a function (called sqlFunction) to calculate the results of an input SQL query using the 'dbFetch' and 'dbSendQuery' functions from the DBI package.\
4.) Outputs the results of each query into a list called 'output' using a second for loop that calls 'sqlFunction' for each question string in 'sqlList'.
...
##joinData
joinedData <- inner_join(flights, airlines,"carrier") %>% 
  na.omit() %>%
...
ggplot(fullData, aes(x=reorder(name,-avgDelay),y=avgDelay, fill=Type)) + 
    ...
    theme(axis.title = element_text(color="#000000", face="bold", size=18),
          axis.text.x = element_text(face="bold", color="#000000", size=16, angle=55, hjust=1),
